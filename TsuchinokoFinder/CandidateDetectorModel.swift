...
final class CandidateDetectorViewModel: NSObject, ObservableObject {
    ...
    @Published var threshold: Double = {
        ...
        return saved == 0 ? 0.72 : max(saved, 0.50)
    }() {
        didSet {
            let clamped = min(max(threshold, 0.50), 0.99)
    ...
    private let requiredCandidateStreak = 2
    private let minimumMotionScore = 0.0
    private let minimumShapeScore = 0.30
    ...
    private func handleVisionResult(_ results: [Any]?) {
        ...
        DispatchQueue.main.async {
            ...
            let isStrongCandidate = calibratedConfidence >= self.threshold && hasCandidateShape
            ...
            } else if confidence >= 0.90 && !hasCandidateShape {
                self.candidateLabel = Copy.noCandidate
